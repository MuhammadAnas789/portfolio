...
export default function About() {
  return (
    <section id="about" className="py-24 max-w-6xl mx-auto px-6">
      ...
      {/* Bio text — full width, constrained */}
      <motion.div
        initial={{ opacity: 0, x: -20 }}
        whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.6, delay: 0.1 }}
        className="border-l-2 border-[#2dd4bf]/30 pl-5 space-y-4 text-[#94a3b8] leading-relaxed max-w-2xl mb-12"
      >
        <p>
      ...
      {/* Stat cards — 3-col grid on desktop, stacked on mobile */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((stat, i) => (
          <motion.div
            ...
            whileHover={{ y: -4 }}
            className="p-6 rounded-lg bg-slate-800/50 backdrop-blur-sm border-t-2 border-teal-400 hover:bg-slate-800 transition-all duration-300 cursor-default"
          >
